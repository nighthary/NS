...
const buildInfo = {
  outputDir: packageInfo.buildInfo.outPath + '-dist'
}
...
gulp.task('copyFront', function () {
  return gulp.src([`${packageInfo.buildInfo.outPath}/**/*`])
    .pipe(gulp.dest(`${buildInfo.outputDir}/${packageInfo.buildInfo.outPath}`));
})

gulp.task('copyBase', function () {
  return gulp.src(['package.json', 'start.js', 'ns.json'])
    .pipe(gulp.dest(`${buildInfo.outputDir}/`));
})
...
gulp.task('copyShell', function () {
  return gulp.src(['./www-localize-start'])
    .pipe(rename('www'))
    .pipe(gulp.dest(`${buildInfo.outputDir}`));
})

gulp.task('copy', gulp.series('clean', 'copyBase', 'copyNode', 'copyFront', 'copyGulp', 'copyShell'), function () {
